...
lido_environment.workflow[0] = (function(){
	...
	my.init = function(view){
        ...
        dom.h3(div, "XML importieren");  /*
        dom.a(div, "", "",  undefined, "Load a document from the database.", function(){ my.parent.showIdentifierSelect();}
        );*/
		var XML_Import = my.fileSimpleInput(div, "input", my.openXML );
		/*newlink = document.createElement('a');
		newlink.setAttribute('class', 'signature');
		newlink.setAttribute('href',"https://portal.dnb.de/opac.htm?method=showSearchForm#top");*/
	...
	my.openXML = function(file){
	
			var xml = file[0];
			
			var text = "";

			readFile(xml, function(e){
				text = e.target.result;
				//console.log("innen" + text);
				var parser = new DOMParser();
				var xmlDoc = parser.parseFromString(text, "application/xml");

				console.log(xmlDoc);
				my.parent.importLIDOXML(xmlDoc);
			...
			function readFile(file, onLoadCallback) {
				...
				readerX.onload = onLoadCallback;
				readerX.readAsText(xml);
			};
	...
	my.fileSimpleInput = function(parent, id, onchange){
	
		var input = dom.input(parent,"xml_input","","xml_input", "file");
		input.multiple = false;
	

		g('xml_input').addEventListener('change', function(evt){
